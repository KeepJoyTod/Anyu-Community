...
<template>
	<view class="complaint-page">
		...
		<view class="nav-bar">
			...
			<text class="nav-title">投诉反馈</text>
		</view>
		...
		<view class="form-card">
			...
			<view class="form-item">
				<text class="label">投诉标题</text>
				<input class="input" v-model="form.title" placeholder="请简要概括投诉内容（必填）" placeholder-class="placeholder" />
			</view>

			<!-- 内容 -->
			<view class="form-item">
				<text class="label">投诉详情</text>
				<textarea class="textarea" v-model="form.content" placeholder="请详细描述问题，包括时间、地点、具体情况等（必填）"
			...
			<view class="form-item">
				...
				<input class="input" v-model="form.contact" placeholder="手机号/邮箱（方便我们回复您）"
					placeholder-class="placeholder" />
			</view>
		...
		<!-- 底部提示 -->
		<view class="tips">
			<text class="tips-text">* 我们将严格保密您的信息，并在1-3个工作日内处理</text>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad() {
			// 确保当前选项卡高亮
			this.currentTab = 'complaint';
		},
		methods: {
			switchTab(tab) {
				if (tab === 'complaint') return; // 已在当前页面
				if (tab === 'suggestion') {
					uni.redirectTo({
